...
def ask_gemini_to_fix(filename, error_msg):
    ...
    # ⭐ 가장 고전적이고 확실한 모델명 'gemini-pro'와 'v1' 경로 조합
    endpoint = f"/v1/models/gemini-pro:generateContent?key={GEMINI_API_KEY}"
    ...
    try:
        ...
        if 'candidates' in data:
            ...
            # ❌ 만약 이것도 안 되면 응답 전문을 다시 확인
            print(f"❌ API 최종 실패: {response_text}")
            sys.exit(1)
...
def run_and_fix(filename):
    ...
    if result.returncode == 0:
        ...
        print(f"🛠️ {filename} 자가 수리 완료. 재검증...")
